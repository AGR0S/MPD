...
import isel.mpd.painter2.app.App;
...
public class PainterFrame extends JFrame {
    ...
    private void setDrawer (String name) {
        if (name == SHAPE_CMD_RECT) {
            currDrawer = new RectDrawer(this);
        }
        if (name == SHAPE_CMD_TRIANGLE) {
            currDrawer = new TriangleDrawer(this);
        }
        if (name == SHAPE_CMD_LINE) {
            currDrawer = new LineDrawer(this);
        }
        if (name == SHAPE_CMD_OVAL) {
            currDrawer = new OvalDrawer(this);
        }
        if (name == SHAPE_CMD_CIRCLE) {
            currDrawer = new CircleDrawer(this);
        }
    }
